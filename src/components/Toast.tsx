...
export const miroToast = {
  ...
  uploadError: (error: string) => {
    toast.error("アップロード失敗", error, 8000);
  },
  searchSuccess: (count: number) => {
    toast.success("検索完了", `${count}件の結果が見つかりました。`, 4000);
  },
  searchError: (error: string) => {
    toast.error("検索失敗", error, 6000);
  },
  ...
  boardLoadError: (error: string) => {
    toast.error("ボード読み込み失敗", error, 6000);
  },
  authError: () => {
    toast.error(
      "認証エラー",
  ...
  rateLimitError: (retryAfter?: number) => {
    ...
    toast.warning("API制限に達しました", message, 8000);
  },
  networkError: () => {
    toast.error(
      "ネットワークエラー",
