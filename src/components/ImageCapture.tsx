...
export function ImageCapture({
  ...
  const playVideoWithRetry = useCallback(
    async (video: HTMLVideoElement, maxRetries: number = 3): Promise<void> => {
      for (let i = 0; i < maxRetries; i++) {
        try {
          ...
          if (i === maxRetries - 1) {
            throw new Error(
              `Video play failed after ${maxRetries} attempts: ${playError}`,
            );
          }
          // 少し待ってからリトライ
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      for (let i = 0; i < constraintAttempts.length; i++) {
        try {
          console.log(
            `Attempting camera access (${i + 1}/${constraintAttempts.length}):`,
      ...
      if (error instanceof Error) {
        if (error.name === "NotAllowedError") {
          ...
          errorMessage =
            "お使いのブラウザはカメラ機能をサポートしていません。Chrome、Firefox、Safari等の最新ブラウザをご利用ください。";
        } else if (error.message === "Camera requires HTTPS") {
          errorMessage =
            "カメラアクセスエラーが発生しました。ブラウザの権限設定を確認してください。";
        } else if (error.name === "OverconstrainedError") {
          errorMessage =
            "カメラの設定に問題があります。別の解像度で再試行します。";
          // より緩い制約で再試行
      ...
      try {
        ...
        for (const file of newFiles) {
          ...
          if (validation.isValid) {
            ...
            clientSkippedFiles.push({
              file,
              error: validation.reason || "不明なエラー",
            });
          }
        }
        ...
        if (clientSkippedFiles.length > 0) {
          const skippedMessages = clientSkippedFiles.map(
            (f) => `${f.file.name}: ${f.error}`,
          );
          alert(
            `以下のファイルがサイズ制限によりスキップされました:\n${skippedMessages.join("\n")}`,
          );
        }
        ...
        const { validFiles, invalidFiles } =
          await validateMultipleFiles(clientValidFiles);
                ...
                <div className="mt-3">
                  ...
                  <div className="p-3 bg-red-50 border border-red-200 rounded-md text-sm text-red-800">
                    ...
                    <ul className="text-left space-y-1">
                      <li>
                        •
                        ブラウザのアドレスバーでカメラアイコンをクリックして許可してください
                      </li>
                      <li>• ページを再読み込みしてもう一度お試しください</li>
                      <li>
                        •
                        他のアプリ（Zoom、Teams等）でカメラを使用していないか確認してください
                      </li>
                      <li>
                        •
                      ...
                      <li>
                        •
                        モバイルの場合：ブラウザの設定でカメラアクセスを許可してください
                      </li>
                    </ul>
                  </div>
                </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {files.map((fileWithPreview) => (
              <div key={fileWithPreview.id} className="relative group card p-2">
                <Image
