 ...
 */
export function handleAndNotifyMiroError(
  error: unknown,
  context?: string,
): UserFriendlyError {
  const userError = ErrorHandler.handleMiroApiError(error);
  ...
  if (error && typeof error === "object" && "response" in error) {
    ...
    if (status === 401) {
      getNotificationService().showError("認証エラー", userError.message, 8000);
    } else if (status === 403) {
 ...
 */
export function handleAndNotifyUploadError(
  error: unknown,
  context?: string,
): UserFriendlyError {
  const userError = ErrorHandler.handleFileUploadError(error);
 ...
 * 検索エラーのハンドリングと通知
 */
export function handleAndNotifySearchError(
  error: unknown,
  context?: string,
): UserFriendlyError {
  const userError = ErrorHandler.handleGenericError(error);
  ...
  getNotificationService().showError("検索エラー", userError.message, 6000);
 ...
 * 一般的なエラーのハンドリングと通知
 */
export function handleAndNotifyGenericError(
  error: unknown,
  context?: string,
): UserFriendlyError {
  const userError = ErrorHandler.handleGenericError(error);
