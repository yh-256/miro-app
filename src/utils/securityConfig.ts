 ...
 * プリフライトリクエストのチェック
 */
export function isPreflightRequest(method: string, headers: Record<string, string | undefined>): boolean {
 ...
 * レート制限のキー生成
 */
export function generateRateLimitKey(req: { headers: Record<string, string | string[] | undefined>; connection?: { remoteAddress?: string } }): string {
  // IP アドレスベースのレート制限
  const forwarded = req.headers['x-forwarded-for'];
  const ip = forwarded ? (typeof forwarded === 'string' ? forwarded.split(',')[0].trim() : forwarded[0]) : 
...
export function validateSecurityConfig(): { isValid: boolean; errors: string[] } {
  ...
  if (config.cors.allowedOrigins.includes('*') && config.cors.credentials) {
    errors.push('CORS: credentials が true の場合、origin に * は使用できません');
  }
  ...
  // HTTPS強制の確認（本番環境）
  if (process.env.NODE_ENV === 'production' && !config.headers.strictTransportSecurity) {
    errors.push('Production: Strict-Transport-Security ヘッダーが設定されていません');
  }
