...
export async function GET(
  ...
  try {
    ...
    const status = userError.message.includes("閲覧するには") ? 403 : 500;
    return NextResponse.json(
...
export async function POST(
  ...
  try {
    ...
    const updatedContext = await loadProblemAccessContext(problemId, userId);
    ...
    return NextResponse.json({
      ...
      progress: updatedContext
        ? toSnapshot(
            ...
            updatedContext.progress ?? undefined,
          )
        : undefined,
    });
  } catch (error) {
